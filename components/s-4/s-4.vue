  ...
  <div class="tournament-bracket">
    <div class="bracket-container">
      ...
      <div class="bracket-round bracket-round-2 s-4">
        <div class="bracket-match" v-for="(m, idx) in semi" :key="'sf' + idx"
          :class="{ clickable: canStart(m), starting: startingId === m.id }" @click="handleStart(m)"
          :title="canStart(m) ? 'Нажмите, чтобы запустить матч' : ''">
          <div class="team-box">
            <img v-if="hasTeam(m.t1)" :src="m.t1.logo || defaultLogo" :alt="m.t1.name || '—'">
            <span>{{ teamName(m.t1) }}</span>
          </div>
          <div class="team-box">
            <img v-if="hasTeam(m.t2)" :src="m.t2.logo || defaultLogo" :alt="m.t2.name || '—'">
            <span>{{ teamName(m.t2) }}</span>
          </div>
      ...
      <!-- Финал -->
      <div class="bracket-round bracket-round-3 bracket-round-3-s4"
        :class="{ clickable: canStart(finalMatch), starting: startingId === finalMatch.id }"
        @click="handleStart(finalMatch)" :title="canStart(finalMatch) ? 'Нажмите, чтобы запустить матч' : ''">
        <div class="bracket-match">
          <div class="team-box">
            <img v-if="hasTeam(finalMatch.t1)" :src="finalMatch.t1.logo || defaultLogo" :alt="finalMatch.t1.name || '—'">
            <span>{{ teamName(finalMatch.t1) }}</span>
          </div>
          <div class="team-box">
            <img v-if="hasTeam(finalMatch.t2)" :src="finalMatch.t2.logo || defaultLogo" :alt="finalMatch.t2.name || '—'">
            <span>{{ teamName(finalMatch.t2) }}</span>
          </div>
      ...
      <div class="bracket-winner">
        <div class="team-box winner">
          <img v-if="hasTeam(winner)" :src="winner.logo || defaultLogo" :alt="winner.name || '—'">
          <span>{{ teamName(winner) }}</span>
        </div>
      </div>
    </div>
...
function hasTeam(t?: TeamUI) {
  if (!t) return false
  const name = String(t.name || '').trim()
  const logo = String(t.logo || '').trim()
  return !!name || !!logo
}
function teamName(t?: TeamUI) {
  return hasTeam(t) ? (t?.name || '—') : 'Ожидание'
}
